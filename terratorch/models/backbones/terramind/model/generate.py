...
def init_conditioned_target_modality(mod_dict, modality_info, domain, num_target_tokens, eos_id=3, s1_id=5):
    ...
    if modality_info[domain]["type"] in ["seq", "seq_token"]:
        ...
        mod_dict["tensor"] = torch.cat(
            [mod_dict["tensor"], torch.zeros((batch_size, num_target_tokens), dtype=torch.int, device=device)], dim=1
        )
        mod_dict["input_mask"] = torch.cat(
            [mod_dict["input_mask"], torch.ones((batch_size, num_target_tokens), dtype=torch.bool, device=device)],
            dim=1,
        )
        mod_dict["target_mask"] = torch.cat(
            [mod_dict["target_mask"], torch.zeros((batch_size, num_target_tokens), dtype=torch.bool, device=device)],
            dim=1,
        )
...
class GenerationSampler(nn.Module):
    ...
    def forward_enc_dec_maskgit_batched(self, mod_dict, target_mod, seed=None):
        ...
        encoder_tokens, encoder_emb, encoder_mask, encoder_mod_mask = self.forward_mask_encoder_generation(
            encoder_mod_dict
        )
        ...
        decoder_tokens, decoder_emb, decoder_mask, decoder_mod_mask, mod_pos = self.forward_mask_decoder_maskgit(
    ...
    def maskgit_step_batched(self, mod_dict, target_mod, num_select, temperature, top_k, top_p, seed=None):
        ...
        mod_dict[target_mod]["input_mask"] = torch.scatter(
            mod_dict[target_mod]["input_mask"], -1, top_pos, torch.zeros_like(top_samples, dtype=torch.bool)
        )
        mod_dict[target_mod]["target_mask"] = torch.scatter(
            mod_dict[target_mod]["target_mask"], -1, top_pos, torch.ones_like(top_samples, dtype=torch.bool)
        )
    ...
    def guided_maskgit_step_batched(
        ...
        top_samples, top_indices, all_samples = self.select_tokens_batched(
            logits, num_select, temperature=temperature, top_k=top_k, top_p=top_p, return_all_samples=True
        )
    ...
    def multi_guided_maskgit_step_batched(
        ...
        logits = logits_uncond + torch.stack(
            [w * (logits_cond - logits_uncond) for w, logits_cond in zip(cond_weights, logits_cond_all)]
        ).sum(dim=0)
        ...
        uncond_dict[target_mod]["input_mask"] = torch.scatter(
            uncond_dict[target_mod]["input_mask"], -1, top_pos, torch.zeros_like(top_samples, dtype=torch.bool)
        )
        uncond_dict[target_mod]["target_mask"] = torch.scatter(
            uncond_dict[target_mod]["target_mask"], -1, top_pos, torch.ones_like(top_samples, dtype=torch.bool)
        )
        ...
        for i in range(len(cond_dicts)):
            ...
            cond_dicts[i][target_mod]["input_mask"] = torch.scatter(
                cond_dicts[i][target_mod]["input_mask"], -1, top_pos, torch.zeros_like(top_samples, dtype=torch.bool)
            )
            cond_dicts[i][target_mod]["target_mask"] = torch.scatter(
                cond_dicts[i][target_mod]["target_mask"], -1, top_pos, torch.ones_like(top_samples, dtype=torch.bool)
            )
    ...
    def forward_enc_dec_roar_batched(self, mod_dict, target_mod, num_select, seed=None):
        ...
        context = self.model.decoder_proj_context(x) + encoder_emb
        decoder_mod_dict = {target_mod: self.model.decoder_embeddings[target_mod].forward_embed(mod_dict[target_mod])}
        decoder_tokens, decoder_emb, decoder_mask, decoder_mod_mask, mod_pos = self.forward_mask_decoder_roar(
    ...
    def multi_guided_roar_step_batched(
        self, uncond_dict, cond_dicts, cond_weights, target_mod, num_select, temperature, top_k, top_p, seed=None
    ):
        ...
        uncond_dict[target_mod]["input_mask"] = torch.scatter(
            uncond_dict[target_mod]["input_mask"], -1, select_pos, torch.zeros_like(samples, dtype=torch.bool)
        )
        uncond_dict[target_mod]["target_mask"] = torch.scatter(
            uncond_dict[target_mod]["target_mask"], -1, select_pos, torch.ones_like(samples, dtype=torch.bool)
        )
        # Update conditioning dicts
        for i in range(len(cond_dicts)):
            cond_dicts[i][target_mod]["tensor"] = torch.scatter(
            ...
            cond_dicts[i][target_mod]["input_mask"] = torch.scatter(
                cond_dicts[i][target_mod]["input_mask"], -1, select_pos, torch.ones_like(samples, dtype=torch.bool)
            )
            cond_dicts[i][target_mod]["target_mask"] = torch.scatter(
                cond_dicts[i][target_mod]["target_mask"], -1, select_pos, torch.zeros_like(samples, dtype=torch.bool)
            )
    ...
    def autoregressive_step_batched(
        ...
        context = self.model.decoder_proj_context(x) + encoder_emb
        decoder_mod_dict = {target_mod: self.model.decoder_embeddings[target_mod].forward_embed(mod_dict[target_mod])}
        decoder_ids, decoder_emb, decoder_mask, decoder_mod_mask, mod_pos = self.forward_mask_decoder_autoregressive(
            decoder_mod_dict, target_mod, seed=seed
        )
    ...
    def guided_autoregressive_step_batched(
        ...
        encoder_tokens, encoder_emb, encoder_mask_cond, encoder_mod_mask = self.forward_mask_encoder_generation(
            encoder_mod_dict
        )
        x = encoder_tokens + encoder_emb
        ...
        encoder_tokens, encoder_emb, encoder_mask_uncond, encoder_mod_mask = self.forward_mask_encoder_generation(
            encoder_mod_dict
        )
        x = encoder_tokens + encoder_emb
            ...
            if self.model.modality_info[target_mod]["type"] == "img":
                ...
                if scheme.lower() == "maskgit":
                    if cfg_scale == 1.0 or len(cfg_conditioning) == 0:
                        mod_dict = self.maskgit_step_batched(
                            mod_dict, target_mod, num_select, temperature=temp, top_k=top_k, top_p=top_p, seed=seed_i
                        )
                    else:
                ...
                if cfg_scale == 1.0 or len(cfg_conditioning) == 0:
                    mod_dict = self.autoregressive_step_batched(
                        ...
                        top_k=top_k,
                        top_p=top_p,
                        text_tokenizer=tokenizer[target_mod].text_tokenizer,
                        seed=seed_i,
                    )
                else:
    ...
    def generate_multi_guided(
        self, uncond_dict, cond_dicts, schedule, top_k=0.0, top_p=0.0, text_tokenizer=None, verbose=False, seed=None
    ):
